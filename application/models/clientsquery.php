...
class Clientsquery extends CI_Model{
    ...
    public function load_managers(){
        ...
        foreach($query->result() as $row){
            ...
            $id=$row->id;
            $output.="<option value=\"$id\">$manager</option>";
        }
        return $output;
    }
    
    public function load_mymanager($id){
        
        $this->db->where('id',$id);
        $query=$this->db->get('zilla_users',1,0);
        foreach($query->result() as $row){
            $this->db->where('id',$row->manager_id);
            $query2=$this->db->get('zilla_users',1,0);
            foreach($query2->result() as $row2){
                return "<option value=\"$row->id\">$row2->username</option><option value=\"0\">NO MANAGER</option>";
                die();
            }
        }
        return "<option value=\"0\">NO MANAGER</option>";
    }
    ...
    public function load_client(){
        ...
        $query=$this->db->get('zilla_users');
        foreach($query->result() as $row){
        $data=array('username'=>$row->username,'pword'=>$row->password,'ofis'=>$row->company , 'email'=>$row->email,'fname'=>$row->name,'subs'=>$row->podpiska,'tel'=>$row->tel,'manager_id'=>$row->manager_id);
        }
        return $data;
    }
    //loads offices
    ...
    public function saveedit(){
        ...
        $data=array('username'=>$user,'password'=>$pass,'name'=>$name,'company'=>$ofis,'manager_id'=>$manager);
        $this->db->where('id',$id);
	...
	$searchdata=array();
        ...
        public function submitfunds(){
            ...
            $creditfinal=0;
            foreach($query->result() as $row){
                $creditamt=$row->podpiska;
                $creditfinal=$creditamt+$credit;
            }
            $data=array('podpiska'=>$creditfinal);
            $this->db->where('id',$id);
            $this->db->update('zilla_users',$data);
            $datenow=date('Y-m-d',time());
            $data=array('Date'=>$datenow,'UserId'=>$id,'CardType'=>'Added Funds From Administrator','CardHolder'=>'System','Amt'=>$credit);
