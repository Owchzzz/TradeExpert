...
class Zilla_auth_model extends CI_Model{
            ...
            if($row=$query->result()){
               ...
               foreach($query->result() as $row){
                ...
                if($data['password'] == md5($this->input->post('password'))){
					...
					if($data['group'] == 0){
						redirect('/zillaauth/login?err=unconfirmedemail','refresh');
                    ...
                    //Invalid Password
                    redirect('/zillaauth/login?err=invalidpass');
                ...
                //Invalid Username
                redirect('/zillaauth/login?err=invaliduser','refresh');
    ...
    public function register($user,$pass,$email=NULL,$defaultuser=0,$dataparam=NULL){
        ...
        if($defaultuser==0){
            redirect('/zillaauth/registersuccess?email='.$email,'refresh');
       // header('location: /zillaauth/registersuccess?email='.$email);
        }
        else{
            redirect('/zillaauth/registersuccessspecial/','refresh');
...
$this->output->set_header("Pragma: no-cache");
    ...
    public function sendconfirmation($email){
        ...
        $msg.="http://$sitename/zillaauth/emailconfirm.html?email=$email&code=$confirmationcode";
        $this->db->where('email',$email);
    ...
    public function recieveconfirmation(){
        ...
        redirect('/zillaauth/confirmationsuccess/','refresh');
